...
def evaluate_strict(preds, golds, output_path=None, length=None, lengths=None):
    """
    ...
    for i, pred in enumerate(preds):
        if output_path is not None:
            op=op+('Actual: ' + str(golds[i]) + '|| Predicted: '+str(pred) + "\n")
...
def evaluate_weak(preds, golds, output_path=None, length=None, lengths=None):
    """
...
def evaluate(preds, golds, label_list=None, output_path=None):
    ...
    if label_list is not None:
        preds = [[label_list[i] for i,val in enumerate(l) if val>0] for l in preds]
        golds = [[label_list[i] for i,val in enumerate(l) if val>0] for l in golds]
    result_strict = evaluate_strict(preds, golds, output_path)
    result_weak = evaluate_weak(preds, golds, output_path)
